...
function censor() {
  const strs = {};
  function change(firstStr, secondStr) {
    if (secondStr) {
      strs[firstStr] = secondStr;
    } else {
      for (const key in strs) {
        const value = strs[key];

        firstStr = firstStr.replace(key, value);
      }
      return firstStr;
    }
  }
...
function makeFuncTester(arrOfTests) {
  function test(callback) {
    for (const arr of arrOfTests) {
      // const testString = callback(arr[0])
      if (testString != arr[1]) {
        return false;
      }
    }
